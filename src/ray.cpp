...
namespace raytracer {
	...
	bool Ray::intersects(const AABB &a, glm::vec3 &result) const
	{
		...
		for(int i = 0; i < 3; i++)
		{
			...
			else
			{
				...
				if (t1 > tmin)
					tmin = t1;	// furthest intersection from a "close" plane

				if (t2 < tmax)
					tmax = t2; // furthest intersection from a "far" plane
	...
	Matrix4 Ray::toMatrix4() const
	{
		Quaternion q = rotationBetweenVectors(Vector3(0.0f, 0.0f, -1.0f), n);
		Matrix4 mat = glm::mat4_cast(q);
		mat[3][0] = p.x;
		mat[3][1] = p.y;
		mat[3][2] = p.z;
		return mat;
