...
namespace raytracer {
	...
	void AABBTransform(AABB &aabb, const Matrix4 &mat)
	{
		...
		// the old aabb extents
		Vector3 e = aabb.max - center;
		// compute the transformed center
		Vector3 newCenter = Vector3(mat * Vector4(center, 1.0f)); 
		// compute positive matrix coefficents
		Matrix3 absMat;
		for(int i = 0; i < 3; i++)
			for(int j = 0; j < 3; j++)
				absMat[i][j] = abs(mat[i][j]);
		// compute the transformed positive extents
		e = absMat * e;

		aabb = AABB(newCenter + e, newCenter - e);
	}
